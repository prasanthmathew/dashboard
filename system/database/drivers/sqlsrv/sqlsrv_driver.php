    ...
    function _execute($sql) {
        if ($this->conn_id) {
            ...
            if (stripos($sql, 'UPDATE') !== FALSE || stripos($sql, 'INSERT') !== FALSE) {
                return sqlsrv_query($this->conn_id, $sql, null, array('Scrollable' => SQLSRV_CURSOR_STATIC));
            }
...
//    }
     ...
     * 
     * @param string $sql
     * @param type $offset 3 (count / page)
     * @param type $limit 2  (page no)
     * @return boolean
     */
    function _limit($sql, $offset, $limit) {
        ...
        $start = $limit == null ? 0 : $limit;
        $end = $start + $offset;


        $replace = preg_replace('/(^\SELECT (DISTINCT)?)/i', '\\1 ROW_NUMBER() OVER (ORDER BY id DESC) AS RowNum,', $sql);

        $sql = "SELECT * FROM (" . $replace . ") as RowContrainedResult " .
                "WHERE RowNum > " . strval($start) . " AND RowNum <= " . $end . " ORDER BY RowNum";
